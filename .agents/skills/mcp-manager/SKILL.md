---
name: mcp-manager
description: 管理和配置 Codex 的 MCP 服务器。当用户提到 MCP、mcp服务器、添加mcp、配置mcp、mcp list、mcp status、安装mcp、新增mcp、mcp认证失败、mcp连接问题等关键词时使用此技能。覆盖添加 stdio 和 HTTP 类型的 MCP 服务器、删除、查看状态、排查认证和环境变量问题。
---

# MCP 服务器管理

管理 Codex 的 MCP（Model Context Protocol）服务器配置。

## 核心原则

**配置文件优先级**：Codex 实际读取的是 `~/.Codex.json` 中的 `mcpServers` 节点，而不是 `.mcp.json`。`Codex mcp add` 命令也写入 `.Codex.json`。如果两个文件存在冲突，`.Codex.json` 优先。

**Windows 特殊格式**：在 Windows 上，stdio 类型的 MCP 服务器必须使用 `"command": "cmd"` + `"args": ["/c", "npx", ...]` 格式，不能直接用 `"command": "npx"`。

**环境变量展开规则**（仅对 `.Codex.json` 有效）：
...
用户级配置：`~/.Codex.json`
- Windows: `C:\Users\<用户名>\.Codex.json`
...
读取 `~/.Codex.json`，在 `mcpServers` 对象中添加新条目。
...
**不要**把实际 key 值硬编码在配置文件中。先在 `~/.Codex/settings.json` 的 `env` 节点定义变量，再通过 `${}` 引用。
...
### 方式二：Codex mcp add 命令

```bash
# stdio 类型（Windows 会自动添加 cmd /c）
Codex mcp add --transport stdio --scope user <名称> -- npx -y <npm包名>@latest

# HTTP 类型
Codex mcp add --transport http --scope user <名称> <URL>

# 带环境变量（stdio 类型）
Codex mcp add --transport stdio --scope user -e KEY=value <名称> -- npx -y <npm包名>@latest
```

注意：`Codex mcp add` 写入 `.Codex.json`。
...
从 `.Codex.json` 的 `mcpServers` 节点中删除对应条目。
...
### 症状：修改了配置但不生效
1. 确认修改的是 `~/.Codex.json`，而不是 `.mcp.json`
2. 完全退出 Codex 进程后重新启动（不是清空对话）
3. 运行 `Codex mcp list` 确认状态
...
可能 `.Codex.json` 中有旧配置覆盖了新配置。检查是否有多余条目。

### 症状：工具不出现但 mcp list 显示 Connected
重启 Codex 进程。MCP 工具列表在启动时加载。
